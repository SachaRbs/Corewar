...
/*   Updated: 2019/10/14 11:25:27 by crfernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				unsigned long long number)
{
	...
	{
		string = ft_strnjoin_printf(ft_dupexit("0"), string, 1, g_str_len);
		g_str_len++;
	}
	if ((CONVERSION == 'x' || CONVERSION == 'X') && number != 0)
	{
		string = ft_strnjoin_printf(ft_dupexit("0X"), string, 2, g_str_len);
		g_str_len += 2;
	}
	return (string);
}
				...
				unsigned long long number, char width_filler)
{
	if (WIDTH > g_str_len)
	{
		if (ft_strchr(FLAGS, '-') == NULL)
			string = ft_strnjoin_printf(ft_nctoaexit(width_filler, WIDTH -
			g_str_len), string, WIDTH - g_str_len, g_str_len);
		else
			string = ft_strnjoin_printf(string, ft_nctoaexit(width_filler,
				...
				unsigned long long number)
{
	...
	if (CONVERSION == 'x')
		string = ft_strtolower_printf(string);
	return (string);
}
